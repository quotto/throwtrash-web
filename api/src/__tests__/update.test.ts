...
describe("update",()=>{
    ...
    it("shared_idが設定されていない場合の更新",async()=>{
        ...
        const result = await update(updateRequest) as APIGatewayProxyStructuredResultV2;
    ...
    it("タイムスタンプが一致しない場合はユーザーエラー",async()=>{
        const updateRequest: UpdateRequest = {
            id: "id001",
            description: JSON.stringify([{id: "100", type: "burn", trash_val: "", schedules: [{ type: "weekday", value: "0"}]}]),
            platform: "android",
            timestamp: 0
        };
        const result = await update(updateRequest) as APIGatewayProxyStructuredResultV2;
    ...
    it("登録データが異常の場合はユーザーエラー",async()=>{
        const updateRequest: UpdateRequest = {
            id: "id001",
            description: JSON.stringify([{type: "burn"}]),
            platform: "android",
            timestamp: 1234567
        };
        const result = await update(updateRequest) as APIGatewayProxyStructuredResultV2;
