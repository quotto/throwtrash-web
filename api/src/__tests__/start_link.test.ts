...
describe("start_link",()=>{
    ...
    describe("正常終了：Androidアプリによるアカウントリンク", () => {
        it("開発用スキル", async () => {
            ...
            // TODO: あとで戻す
            const loginMatchRe = /^https:\/\/alexa\.amazon\.com\/spa\/skill-account-linking-consent\?fragment=skill-account-linking-consent&client_id=dummy_client_id&scope=alexa::skills:account_linking&skill_stage=development&response_type=code&state=.{20}&redirect_uri=https:\/\/mobileapp-fluter.mythrowaway.net\/accountlink$/
            expect(loginMatchRe.exec(body.url)).toBeTruthy();
            expect(mockedPutAccountLinkItem).toBeCalledWith(expect.objectContaining({
                token: expect.any(String),
                user_id: "id001",
                ...
                // TODO: あとで戻す
                redirect_url: expect.stringMatching(/^https:\/\/mobileapp-fluter\.mythrowaway\.net\/accountlink$/),
                TTL: expect.any(Number)
            }));
        });
        it("本番用スキル", async () => {
            ...
            // TODO: あとで戻す
            const loginMatchRe = /^https:\/\/alexa\.amazon\.com\/spa\/skill-account-linking-consent\?fragment=skill-account-linking-consent&client_id=dummy_client_id&scope=alexa::skills:account_linking&skill_stage=live&response_type=code&state=.{20}&redirect_uri=https:\/\/mobileapp-fluter.mythrowaway.net\/accountlink$/
            expect(loginMatchRe.exec(body.url)).toBeTruthy();
            expect(mockedPutAccountLinkItem).toBeCalledWith(expect.objectContaining({
                token: expect.any(String),
                user_id: "id001",
